...
const TextBox: React.FC<FormFieldProps> = ({
  ...
  const control: Control<FieldValues> = controlProp as unknown as Control<FieldValues>;
  ...
  const classNames = (...args: (string | number | Record<string, boolean> | (string | number | Record<string, boolean>)[])[]): string => {
    return args
      .map((arg) => {
  ...
  const renderInput = (field: any) => (
    <>
      <input
        ...
        onChange={(e) => {
          if (type === "file") {
            const file = e.target.files ? (e.target.files.length > 1 ? e.target.files : e.target.files[0]) : null;
        ...
        className={classNames({ "is-invalid": !!errors[name] || !!customError }, "form-input ") + (className ? ` ${className}` : "")}
        disabled={disabled}
        placeholder={placeholder || (label ? `Enter your ${label?.toLocaleLowerCase()}` : "")}
  ...
  return (
    <div className={`form-field ${margin} ${width} ${size} ${flexDirection}`}>
      ...
      {showErrorMessage && errors[name] && typeof errors[name] !== "string" && errors[name]!.message && (
